...
PecinhaAux MinhasPecas[240]; //contem as pecas que poderao ser utilizadas durante o jogo, eh sua caixa de blocos de lego; 
...
void adicionarPeca(int, int, int, float);
		...
		case 13: {
		    ...
		    if(girarPeca == -1) {
				...
				if(MinhasPecas[girarPeca].tamp == 1) {
				    ...
				    // Verifica se a pecinha já estava rotacionada
...
void adicionarPeca(int tam, int color, int type, float pos){
	if(verificaPosMatriz(&cursor, posCursor[0], posCursor[1], posCursor[2], tam, 0) == 0) { 
		ocupaPosMatriz(&cursor, posCursor[0], posCursor[1], posCursor[2], tam, 0, pecasadd);
        MinhasPecas[pecasadd]=novaPeca(pecasadd, color, type, tam, 1, 0);
        MinhasPecas[pecasadd]=alteraTransladoPeca((posCursor[0])*2, (posCursor[1]+0.25)*2, (posCursor[2]-pos)*2); //blocotipo0
	    pecasadd++;
